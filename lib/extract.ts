...
export async function processEvent(eventId: number) {
  ...
  const isCloseoutSource = Boolean(event.lead_id && closeoutCompletionEventId)
  const completionStillActive = async () => {
    if (!isCloseoutSource || !event.lead_id || !closeoutCompletionEventId) return true
      ...
      SELECT l.completed_at,
        EXISTS(
          ...
          WHERE done.id = ${closeoutCompletionEventId}::bigint
            ...
            AND done.lead_id = ${event.lead_id}::bigint
        ) AS completion_exists,
        EXISTS(
          SELECT 1 FROM events undone
          WHERE undone.kind IN ('job.completion-undone', 'completion_undone')
            AND undone.lead_id = ${event.lead_id}::bigint
            AND undone.detail->>'completionEventId' = ${String(closeoutCompletionEventId)}::text
        ) AS undone
      FROM leads l WHERE l.id = ${event.lead_id}::bigint LIMIT 1`) as { completed_at: string | null; completion_exists: boolean; undone: boolean }[]
  ...
  const leads = event.lead_id ? (await sql`
    SELECT id, first_name, last_name, service, status, estimate_value_cents, invoice_number, is_test, completed_at
    FROM leads WHERE id = ${event.lead_id}::bigint LIMIT 1`) as Record<string, unknown>[] : []
  ...
  const activeClaims = event.lead_id || event.person_id ? (await sql`
    ...
    WHERE subject_type = ${event.lead_id ? "lead" : "person"}::text
      AND subject_id = ${event.lead_id ?? event.person_id}::bigint
  ...
  if (event.lead_id && event.kind === "form.quote") {
    await sql`UPDATE leads SET crew_message = ${crewSafeBody}::text WHERE id = ${event.lead_id}::bigint`
  }
  if (event.lead_id && event.kind === "note.text") {
    await sql`UPDATE leads SET crew_notes = ${crewSafeBody}::text WHERE id = ${event.lead_id}::bigint`
  }
  ...
  for (const item of object.commitments) {
    if (item.matches_existing_commitment_id && item.marks_existing_as) {
      if (suppliedOpenCommitmentIds.has(item.matches_existing_commitment_id)) {
        await setCommitmentStatus({ id: item.matches_existing_commitment_id, status: item.marks_existing_as, sourceEventId: event.id, leadId: event.lead_id, personId: event.person_id })
      } else {
    ...
    await addCommitment({ leadId: event.lead_id, personId: event.person_id, direction: item.direction, operatorId: event.actor_type === "operator" ? Number(event.actor_id) || null : null, summary: item.summary, crewSummary: redactCrewText(item.crew_safe_summary), dueAt: item.due_at_iso, sourceEventId: event.id, confidence: item.confidence, visibleOnGlass: false, itemKey: `commitment:${itemHash}` })
  }
  for (const fact of object.facts) {
    if (fact.confidence < 0.6 || (!event.lead_id && !event.person_id)) continue
    const factHash = createHash("sha256").update(JSON.stringify({ predicate: fact.predicate, value: fact.value })).digest("hex")
    const claimId = await addClaim({ subjectType: event.lead_id ? "lead" : "person", subjectId: event.lead_id ?? event.person_id!, predicate: fact.predicate, value: fact.value, confidence: fact.confidence, sourceEventId: event.id, extractedBy: AI_MODELS.extraction, itemKey: `fact:${factHash}` })
    if (fact.supersedes_claim_id) {
      await sql`UPDATE claims old SET superseded_by = ${claimId}::bigint
        WHERE old.id = ${fact.supersedes_claim_id}::bigint AND old.superseded_by IS NULL
          AND old.subject_type = ${event.lead_id ? "lead" : "person"}::text
          AND old.subject_id = ${event.lead_id ?? event.person_id}::bigint
          AND old.predicate = ${fact.predicate}::text`
    }
    if (fact.predicate === "quoted_price_cents" && event.lead_id && fact.confidence >= 0.6) {
      const raw = fact.value as number | { cents?: number }
      const cents = typeof raw === "number" ? raw : Number(raw?.cents)
      if (Number.isFinite(cents) && !isTest) await notifyAll({ priority: "digest", stock: "manila", title: `Looks like a $${(cents / 100).toLocaleString("en-US")} quote`, body: "Confirm whether this was quoted.", url: `/ops/leads/${event.lead_id}#quote-capture`, sourceEventId: event.id, ownerOnly: true, actionKind: "quote-capture", actionDetail: { leadId: event.lead_id, claimPredicate: fact.predicate, amountCents: cents } })
    }
  }
    ...
    if (event.lead_id) {
      ...
      await sql`
        UPDATE leads SET
          ...
          updated_at = now()
        WHERE id = ${event.lead_id}::bigint`
      if (update.service) await addClaim({ subjectType: "lead", subjectId: event.lead_id, predicate: "service", value: update.service, confidence: update.confidence, sourceEventId: event.id, extractedBy: AI_MODELS.extraction })
    }
  }
  if (isCloseoutSource && event.lead_id && object.glass_caption_draft) {
    ...
    if (caption && !unsafe) {
      ...
      await sql`
        ...
        VALUES (${event.lead_id}::bigint, ${event.id}::bigint, ${caption}::text, ${captionHash}::text)
        ON CONFLICT (source_event_id, caption_hash) DO NOTHING`
      await sql`
        UPDATE leads SET glass_caption_draft = ${caption}::text, updated_at = now()
        WHERE id = ${event.lead_id}::bigint AND glass_caption_approved_at IS NULL`
      ...
      if (trust[0]?.auto_share) {
        await sql`
          ...
          WHERE id = ${event.lead_id}::bigint AND completed_at IS NOT NULL`
        await sql`
  ...
  if (object.auto_reply_type === "contact_departed" && object.contact_churn && event.person_id) {
    ...
    const churnEventId = await recordEvent({ kind: "contact.churn", actorType: "ai", leadId: event.lead_id, personId: event.person_id, externalId: `${event.id}:contact-churn`, body: `${object.contact_churn.left_name} no longer works with this account`, crewBody: `${object.contact_churn.left_name} no longer works with this account`, detail: { sourceEventId: event.id, evidence: object.contact_churn.evidence, confidence: object.contact_churn.confidence } })
  ...
  if (object.urgency === "interrupt" && urgencyEvidence && !isTest) await notifyAll({ priority: "interrupt", stock: "red", title: "This needs a hand", body: event.body.slice(0, 110), url: event.lead_id ? `/ops/leads/${event.lead_id}` : "/ops", sourceEventId: event.id })
  await markEventProcessed(event.id)
  return { processed: true, commitments: object.commitments.length, facts: object.facts.length }
}
