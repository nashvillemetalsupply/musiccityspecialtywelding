...
const SHORT_DAY = new Intl.DateTimeFormat("en-US", { timeZone: "America/Chicago", month: "short", day: "numeric" })

// The owner's cost-per-lead tile, word for word as Fable ruled it 2026-09-18:
...
export function costPerLeadTile({ monthLabel, totalLeads, channels, spendAsOf, perChannelReady }, nowMs) {
  ...
  const spendCents = recorded.reduce((sum, channel) => sum + channel.spendCents, 0)
  const asOf = spendAsOf ? ` as of ${SHORT_DAY.format(new Date(spendAsOf))}` : ""
  const behind = spendAsOf ? spendDaysBehind(spendAsOf, nowMs) : 0
  const stale = behind > 0 ? ` · spend feed ${behind} days behind` : ""
