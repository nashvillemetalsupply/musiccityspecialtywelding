...
export async function persistLockedBuildSheet({ sql, leadId, operatorId, lockKey, candidate }) {
  const rows = await sql`
    WITH lead_scope AS (
      SELECT l.id AS lead_id FROM leads l JOIN operators o ON o.id = ${operatorId}::bigint
        AND o.role = 'owner' AND o.active = true
      WHERE l.id = ${leadId}::bigint AND l.is_test = true
    ), locked_scope AS MATERIALIZED (
      SELECT scope.lead_id,
        pg_advisory_xact_lock(hashtextextended(
          ('build-sheet:' || scope.lead_id::text || ':' || ${lockKey}::text)::text,
          0
        )) AS acquired
      FROM lead_scope scope
    ), existing AS (
      SELECT s.id, s.sequence, s.snapshot, s.locked_at
      FROM locked_scope scope
      JOIN build_lock_receipts receipt ON receipt.lead_id = scope.lead_id
        AND receipt.lock_key = ${lockKey}::text AND receipt.is_test = true
      ...
      SELECT scope.lead_id, 2::int, true FROM locked_scope scope
      ...
      SELECT sheet.lead_id, ${lockKey}::text, sheet.id, true FROM sheet
      RETURNING build_sheet_id
    )
    SELECT sheet.id, sheet.sequence, sheet.snapshot, sheet.locked_at, true AS inserted
    FROM sheet JOIN receipt ON receipt.build_sheet_id = sheet.id
    UNION ALL
    SELECT existing.id, existing.sequence, existing.snapshot, existing.locked_at, false AS inserted
    FROM existing
    LIMIT 1`
  const sheet = rows[0]
  if (!sheet) throw new Error("The Build Sheet lock is still being filed. Tap once more.")
  return { sheet, inserted: sheet.inserted === true }
}
