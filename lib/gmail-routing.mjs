...
function parseAuthenticationResults(raw) {
  ...
  for (const segment of segments) {
    const head = segment.match(/^\s*([a-z0-9-]+)\s*=\s*([a-z0-9-]+)/i)
    if (!head) continue
    const method = head[1].toLowerCase()
    // Gmail never writes the same method twice; refuse to guess which copy wins.
    if (methods.has(method)) return null
    methods.set(method, { result: head[2].toLowerCase(), properties: clauseProperties(segment.slice(head[0].length)) })
  }
...
function methodDomainIsIntuit(methods, method, keys) {
  const clause = methods.get(method)
  if (!clause || clause.result !== "pass") return false
...
export function isAuthenticatedIntuitPayment({ from, labels = [], authenticationResults = "", subject = "", body = "" }) {
  ...
  const spfPass = methodDomainIsIntuit(parsed.methods, "spf", ["smtp.mailfrom"])
