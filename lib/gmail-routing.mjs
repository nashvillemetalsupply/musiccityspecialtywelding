...
export function extractQuickBooksPaymentFacts({ subject = "", body = "" }) {
  ...
  // would never have matched a job numbered 1357. Real numbers do contain hyphens
  // (INV-1357), so only a trailing separator is trimmed.
  const invoiceNumber = text.match(/Invoice\s*#?\s*([A-Za-z0-9-]+)/i)?.[1]?.replace(/[-\s]+$/, "") || null
