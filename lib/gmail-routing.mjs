...
function splitMailboxList(value) {
  ...
  for (let i = 0; i < value.length; i += 1) {
    const ch = value[i]
    if (ch === "\\" && quoted) { current += ch + (value[i + 1] ?? ""); i += 1; continue }
...
function addressedToShop(recipients) {
  ...
  // than quietly dropping it and judging on the remaining headers.
  return values.every((value) => value !== "" && splitMailboxList(value).every((part) => {
    const address = mailboxAddress(part)
    return address !== "" && domainFromAddress(address) === SHOP_MAIL_DOMAIN
  }))
}

// The address is the angle-address at the END, after quoted display names are
// removed. Reading the first <...> instead lets a display name carry the bait:
...
// RFC 6376 allows whitespace around the "=" in a tag, so matching "l=" literally
...
function dkimTags(signature) {
  ...
  for (const segment of String(signature).split(";")) {
    ...
    tags.set(key, pair[2].trim())
  }
  return tags
}

// Authentication-Results reports that a signature passed; it does not report what
// that signature covered. A signature omitting to: from h= could be readdressed
// after the fact, and one carrying an l= body-length tag lets anyone append text
// that the amount and balance parsers would then read as Intuit's. Both receipts
...
function intuitSignaturesAreFullyBound(signatures) {
  ...
  return intuit.every((tags) => {
    ...
    const covered = (tags.get("h") ?? "").replace(/\s+/g, "").toLowerCase().split(":")
    return covered.includes("to") && covered.includes("subject")
  })
}
...
export function isAuthenticatedIntuitPayment({ from, labels = [], authenticationResults = "", subject = "", body = "", recipients = [], dkimSignatures = [] }) {
  ...
  if (!intuitSignaturesAreFullyBound(dkimSignatures)) return false
