...
function intuitSignaturesAreFullyBound(signatures) {
  const values = (Array.isArray(signatures) ? signatures : [signatures])
    .map((value) => String(value ?? ""))
    .filter((value) => value.trim() !== "")
  const intuit = values.filter((value) => isIntuitDomain(identityDomain(value.match(/[;\s]d=([^;\s]+)/i)?.[1] ?? "")))
  if (intuit.length === 0) return false
  return intuit.every((value) => {
    if (/[;\s]l=\s*\d/i.test(value)) return false
    const covered = (value.match(/[;\s]h=([^;]+)/i)?.[1] ?? "").replace(/\s+/g, "").toLowerCase().split(":")
