...
export function isAuthenticatedIntuitPayment({ from, labels = [], authenticationResults = "", subject = "", body = "" }) {
  ...
  // Gmail reports the signing identity as header.i=@domain and only emits
  // header.d= when the signature carries no i= tag. Requiring header.d= alone
  // rejected all 28 real Intuit receipts between 2026-08-05 and 2026-08-22 --
  // the test fixture below was hand-written to match the regex instead of
  // sampled from a real message, so the suite never saw it. RFC 6376 requires
  // i= to sit inside d=, and the dmarc=pass check still pins alignment.
  // All three clauses end on (?![a-z0-9.-]) rather than : a word boundary
  // is satisfied by the dot in intuit.com.attacker.net, so  accepted any
  // attacker subdomain that merely started with the real domain.
  const dkimPass = /\bdkim=pass\b/.test(auth) && /\bheader\.[di]=@?(?:[a-z0-9-]+\.)*intuit\.com(?![a-z0-9.-])/.test(auth)
  const spfPass = /\bspf=pass\b/.test(auth) && /\bsmtp\.mailfrom=[^\s;@]+@(?:[a-z0-9-]+\.)*intuit\.com(?![a-z0-9.-])/.test(auth)
  const dmarcPass = /\bdmarc=pass\b/.test(auth) && /\bheader\.from=(?:[a-z0-9-]+\.)*intuit\.com(?![a-z0-9.-])/.test(auth)
