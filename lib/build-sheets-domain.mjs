...
function shopMeasurement(claim) {
  ...
  if (["ft", "foot", "feet"].includes(unit)) return `${claim.value} feet`
...
export function deriveBuildDraft({ claims = [], decisions = [] } = {}) {
  ...
  for (const requirement of REQUIRED_GATE_FACTS.filter((fact) => fact.critical)) {
    ...
    if (!matching.length || matching.every((fact) => fact.state === "still-need")) {
      ...
      fabricationBlockers.push(`${FACT_LABELS[requirement.factKey]} is a shop estimate.`)
    } else {
      fabricationBlockers.push(`${FACT_LABELS[requirement.factKey]} is not shop-confirmed.`)
    }
  }

  const referenceConflict = conflicts.find((conflict) => conflict.kind === "unresolved-reference")
  const referenceClaim = referenceConflict
    ? visibleClaims.find((claim) => referenceConflict.claimIds.includes(Number(claim.id)))
    : null
  return {
    claims: visibleClaims,
    decisions: [...activeDecisions.values()],
    conflicts,
    recommendedQuestion: referenceClaim ? {
      question: `Ask if ${shopMeasurement(referenceClaim)} is the opening or the finished gate.`,
