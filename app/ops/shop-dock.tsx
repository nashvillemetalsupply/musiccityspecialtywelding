...
export function ShopDock({ voiceReady }: { voiceReady: boolean }) {
  ...
  return <section className="ops-dock" aria-label="Ask Jobs and Morning Brief">
    ...
    {open === "handset" && <section className="ops-handset-panel" id="ops-handset-panel" aria-labelledby="ops-handset-title">
      ...
      <div className="ops-handset-controls"><VoiceCaptureButton available={voiceReady} className="ops-hold-talk" disabled={working} onError={(message) => { setAnswer(message); setAnswerStatus(message) }} onTranscript={(transcript) => { setQuestion(transcript); void ask(transcript) }} /><form aria-busy={working} onSubmit={(event) => { event.preventDefault(); void ask() }}><label className="jobs-sr-only" htmlFor="ops-handset-question">Question for Ask Jobs</label><input ref={questionRef} id="ops-handset-question" name="question" type="text" autoComplete="off" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Type a question" /><button type="submit" disabled={working}>{working ? "Asking…" : "Ask"}</button></form><label htmlFor="ops-handset-speak"><input id="ops-handset-speak" type="checkbox" checked={speak} onChange={(event) => setSpeak(event.target.checked)} /> Read answer aloud</label></div>
