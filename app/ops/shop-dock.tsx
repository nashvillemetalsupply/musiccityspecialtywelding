...
export function ShopDock({ voiceReady }: { voiceReady: boolean }) {
  ...
  return <section className="ops-dock" aria-label="Ask Jobs and Morning Brief">
    ...
    {open === "handset" && <section className="ops-handset-panel" id="ops-handset-panel" aria-labelledby="ops-handset-title">
      ...
      {(answer || working) && <article className="ops-thermal-slip" aria-busy={working}><span>{working ? "Working..." : "Answer"}</span><p>{withoutEvidenceMarkers(answer) || "Working..."}</p>{receipts.length > 0 && <div><small>Sources</small>{receipts.map((item) => <a href={`/ops?receipt=${item.id}#receipt`} key={item.id}>{shopEventLabel(item.kind)}, {new Date(item.occurred_at).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</a>)}</div>}<button type="button" className="ops-crumple" onClick={() => { setAnswer(""); setAnswerStatus("Answer cleared."); setQuestion(""); setReceipts([]); questionRef.current?.focus() }}>Clear answer</button></article>}
