...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isInteger(personId) || personId <= 0) return { title: "Customer not found · MCSW Jobs" }
  const account = await getAccount(personId, operator.role)
  const name = account?.person.company || account?.person.display_name || "Regular account"
...
export default async function AccountPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ page?: string; q?: string; year?: string }> }) {
  ...
  const paperworkIntentKey = randomUUID()
