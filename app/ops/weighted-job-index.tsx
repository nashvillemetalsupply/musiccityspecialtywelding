...
function boardPageHref(stage: JobBoardStage, query: string, page: number) {
  const params = new URLSearchParams({ board: "v2" })
  if (stage !== "board") params.set("stage", stage)
  if (query.trim()) params.set("q", query.trim())
  if (page > 1) params.set("page", String(page))
  return `/ops?${params.toString()}#active-jobs`
}
...
function SignalStack({ signals }: { signals: BoardJobRow["board_signals"] }) {
  ...
  return <div>
    {signals.map((signal, index) => <div className="jobs-row-detail" key={`${signal.kind}-${index}`}>
      <strong className="jobs-row-stage">{signal.reason}</strong>
      <p>{formatLate(signal.hoursLate)}</p>
    </div>)}
  </div>
}
  ...
  return <article className={`jobs-row${lead.board_stage === "attention" ? " is-attention" : ""}${lead.is_test ? " is-test" : ""}`}>
    <div className="jobs-row-copy">
      <div className="jobs-row-heading">
        ...
        {priorJobs > 0 && <span className="jobs-row-lifecycle">{priorJobs} prior {priorJobs === 1 ? "job" : "jobs"}</span>}
      </div>
      <SignalStack signals={lead.board_signals} />
...
export function WeightedJobIndex({
  leads,
  counts,
  repeatCounts,
  stage,
  ...
  return <section className="jobs-panel jobs-active" id="active-jobs" aria-labelledby="active-jobs-title">
    ...
    {leads.length === 0
      ? <div className="jobs-index-empty">
          ...
          {query && <Link href="/ops?board=v2#active-jobs">Clear search</Link>}
        </div>
