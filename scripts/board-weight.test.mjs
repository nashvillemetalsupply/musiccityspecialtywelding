...
test("the tracker leads the board: first card in main, and there is no pane", () => {
  ...
  // the column beside the sort reads on the sort's own clock
  assert.match(BOARD_SOURCE, /waitingAge\(lead\.created_at, nowMs\)/)
  assert.doesNotMatch(BOARD_SOURCE, /waitingAge\(lead\.board_since/)
