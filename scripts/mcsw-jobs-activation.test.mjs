...
test("work orders use the requested phone-first order and one condensed paged record", () => {
  ...
  const summary = page.indexOf('className="ops-job-jig"')
  const promises = page.indexOf('className="ops-job-promises"')
  const messages = page.indexOf('className="ops-spike"')
  const details = page.indexOf('className="ops-job-details-shell"')
  const done = page.indexOf("<DoneStamp")
  const history = page.indexOf('className="ops-card ops-history ops-recent-activity"')
...
test("voice cutover and customer texting are independent launch gates", () => {
  ...
  assert.match(workOrder, /customerTextReady && <Link className="ops-act-text"/)
