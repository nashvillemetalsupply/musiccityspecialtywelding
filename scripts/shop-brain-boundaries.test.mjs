...
test("private callers cannot starve raw-call recovery", () => {
  ...
  assert.match(ingest, /NOT EXISTS \([\s\S]*kind = 'call\.in'[\s\S]*external_id = calls\.twilio_sid/)
  ...
  assert.match(intake, /persistInboundCallReceipt/)
  assert.match(intake, /'\{"reconciliationHandled":true\}'::jsonb/)
