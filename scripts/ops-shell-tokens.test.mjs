...
test("every legacy colour token the ops rules reference is aliased on the shell", () => {
  const shellBlock = SHELL.slice(SHELL.indexOf(".ops-shell {"), SHELL.indexOf("\n}\n", SHELL.indexOf(".ops-shell {")))
  ...
  assert.deepEqual(missing, [], `legacy tokens used by ops rules but not aliased on .ops-shell: ${missing.join(", ")}`)
